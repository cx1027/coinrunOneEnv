...
public:
  ...
  Maze(const int _w, const int _h, int _game_type)
  {
    ...
    coins = 0;
    printf("******** initializing **********\n");
    printf("******** initializing maze _w **********\n");

    printf("******** initializing maze _h**********\n");
    std::cout << "w:  "<<w<<"h:  "<<h;
...
public:
  ...
  std::set<int> lookup(std::set<int> *cell_sets, int x, int y) {
      printf("class RandomMazeGenerator*****************\n");
      std::cout << "maze Rec x:  "<<x<<"  maze Rec y:  "<<y<<" maze_dim: "<<maze_dim<<"********";
  ...
  {
    maze->fill_elem(0, 0, maze->w, maze->h, SPACE);
    printf("\n initial_floor_and_walls*****************\n");
//    printf("maze->printWalls();\n");
    maze->printWalls();
    if (game_type != CoinRunMaze_v0) {
...
{
  printf("ground_theme_downsample\n");
...
{
  printf("player_theme_downsample\n");
...
{
  printf("    load_resource : relpath     ");
  auto path = resource_path + "/" + relpath;
  auto img = QImage(path);
  std::cout << relpath.toStdString();
  if (img.width() == 0) {
...
//std::ostream& operator<<(std::ostream& os, const GroundTheme& t) {
//   return  os << "theme_name: " << t.theme_name;
////              << "walls: " << t.walls << endl
////              << "default_wall: " << t.default_wall << endl;
//   }
...
//};
    ...
    std::cout <<"t.theme.name: "<<t.theme_name.toStdString()<<" finishe***************\n";
    QString walls = "kenney/Ground/" + t.theme_name + "/" + t.theme_name.toLower();
    std::cout <<"QString walls: "<<walls.toStdString()<<" finishe***************\n";
...
//  printf("struct Agent");
  ...
  void sub_step(float _vx, float _vy)
  {
    ...
    int iy = int(y);
    int inx = int(nx);
    std::cout<<"\nsub_step x,y,nx to ix,iy,inx"<<" x:"<<x<<" y:"<<y<<" nx:"<<nx<<" ix:"<<ix<<" iy:"<<iy<<" inx:"<<inx<<"\n";

//    char aaa=maze->get_elem(inx, iy);
//    printf(aaa);
  ...
  void step_maze(int game_type) {
    ...
    printf("******game_type:%d, nx:ny = %d:%d",game_type, nx, ny);
    eat_coin(x, y);
  }
  ...
  void step_coinrun(int game_type)
  {
    ...
    std::cout<<"\nrun void step_coinrun(int game_type)"<<"\n";
    std::cout<<"\n*******************************in step_coinrun"<<"\n";
    ...
    std::cout<<"\ntest_is_ladder1 = maze->get_elem(near_x, int(y + 0.2))"<<test_is_ladder1<<"\n";
    char test_is_ladder2 = maze->get_elem(near_x, int(y - 0.2));
    std::cout<<"\ntest_is_ladder2 = maze->get_elem(near_x, int(y - 0.2))"<<test_is_ladder2<<"\n";
    ...
    if (ladder_mode) {
      ...
      std::cout<<"\n normal vy=maze->gravity: "<<vy<<"\n";
      vy -= maze->gravity;
    }

    vy = clip_abs(vy, max_jump);
    vx = clip_abs(vx, max_speed);
    std::cout<<"\n vy=clip_abs(vy, max_jump): "<<vy<<"\n";
    std::cout<<"\n vy=clip_abs(vx, max_speed): "<<vx<<"\n";
    ...
    printf("final vx in step_coinrun:");
    std::cout << vx << std::endl;
    printf("final vy in step_coinrun:");
...
//    maze->printWalls();
    ...
    printf("**********************finish step_coinrun:");
  }

  void step(int game_type)
  {
    std::cout<<"\nrun void step(int game_type)"<<"\n";
    time_alive += 1;
    int timeout = 0;
...
{
  printf("initialize_set_monitor_dir      ");
  monitor_dir = d;
...
{
  printf("vec_step_async_discrete");
  std::shared_ptr<VectorOfStates> vstate = vstate_find(handle);
  QMutexLocker sleeplock(&h2s_mutex);
  {
    ...
    for (int e = 0; e < vstate->nenvs; e++) {
      ...
      printf("******action:%d, agent:dx:dy = %d:%d\n",*actions, state->agent.action_dx, state->agent.action_dy);
    }
  }
  ...
  bool* done)
{
  ...
  while (1) {
    ...
    {
        //todo: what is states_mutex???
      QMutexLocker lock2(&vstate->states_mutex);
      for (int e = 0; e < vstate->nenvs; e++) {
        //in each env, get state pointer
        std::shared_ptr<State> state = vstate->states[e];
        //todo: what is step_mutex???
        QMutexLocker lock3(&state->step_mutex);
...
public:
  ...
  TestWindow()
  {
    printf("TestWindow : public QWidget");
    viz = new Viz();
...
{
  printf("extern "" void test_main_loop");
  QApplication *app = 0;
  TestWindow *window = 0;
#ifdef Q_MAC_OS
