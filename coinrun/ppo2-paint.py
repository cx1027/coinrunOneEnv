...
class Model(object):
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        A = train_model.pdtype.sample_placeholder([None])  # advantege?
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for i in range(self.nsteps):
            ...
            # tmp = [self.obs[:][0]]
            #
            # # img_input = img.imgbuffer_process(self.obs[:])
            # img_input = img.imgbuffer_process(tmp, [1024, 1024])
            # # plt.subplot(2, 2, 1)
            # plt.imshow(np.uint8(img_input[:, :, 0] * 255))
            # plt.imsave('bbbb.png', img_input)
            # print("aaaaaaaa")
            ...
            if (i % 50 == 6) and (self.imgNum < 300):
                plt.imsave('%i.png' % (self.imgNum), img_input)
                self.imgNum = self.imgNum + 1
                print("imgNum:%i" % (self.imgNum))

            # self.env.ale.saveScreenPNG(b'test_image.png')
            for info in infos:
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    # every nbatch_train to update Model
    ...
    # one runner runs 256 steps, collect data every 256 steps
    ...
    for update in range(1, nupdates + 1):
        ...
        # runner.run to collect data in 256 steps
